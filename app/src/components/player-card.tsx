...
async function getAiBidSuggestion(payload: {
  player: Player;
  players: Player[];
  currentRound: number;
  startingCardCount: number;
}): Promise<BidSuggestion | { error: string }> {
  try {
    const response = await fetch('/api/ai/get-bid-suggestion', {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      return { error: errorData.error || 'Failed to get suggestion from AI.' };
    }
